...
```{r echo=TRUE}
#import data
sample_df = read.delim("sample10.methyl.filtered.bed", header=F)
hp1 = read.delim("sample10.hap1.methyl.filtered.bed", header=F)
hp2 = read.delim("sample10.hap2.methyl.filtered.bed", header=F)
```


```{r include=FALSE}
#import ctrls
ctrls= read.xlsx("ctrls_2.xlsx",1)
ctrls<- ctrls [, c(2:4)]

#dataset with selected cloumns + filtering
...
GRB10_alt_TSS_DMR<- sorted_df %>% filter(between(start_position,50943355,50944182)) 
...
df <- df %>%
  ...
  kable_styling() %>%
  column_spec(9, color = ifelse(df$"Mean methylation" < 25 | df$"Mean methylation" > 75, "red", "black"),
              bold = ifelse(df$"Mean methylation" < 25 | df$"Mean methylation" > 75, TRUE, FALSE))
